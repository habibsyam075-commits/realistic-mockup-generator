...
const ResultDisplay: React.FC<ResultDisplayProps> = ({ mockupResult, onReset, onRegenerate, onAdjust }) => {
  const { t } = useLanguage();
  
  const handleDownload = () => {
    const link = document.createElement('a');
    link.href = mockupResult;
    link.download = 'wallet-mockup.png';
  ...
  return (
    <div className="w-full flex flex-col items-center text-center">
      <h2 className="text-2xl font-bold text-white mb-4">{t('resultTitle')}</h2>
      <p className="text-gray-400 mb-6">{t('resultImageSubTitle')}</p>
      
      <div className="w-full max-w-xl p-2 bg-gray-700 rounded-lg shadow-lg">
        <img src={mockupResult} alt={t('generatedMockup')} className="w-full h-auto rounded-md" />
      </div>
